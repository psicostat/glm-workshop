...
## Categorical predictor, confidence intervals
...
This means that we have an odds ratio of `r round(exp(coef(fit_int_emo)[3]), 2)` in favour of anger and an odds ratio of `r round(exp(coef(fit_int_emo)[2]), 2)` for a unit increase in intensity.
...
## When using the probit link function?
