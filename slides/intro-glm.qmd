...
p = ggplot(dd, aes(x = x, y = mean)) +
  geom_line(size = 3,color = my_cols[2]) +
  xlab("x") +
  ylab("y") +
  scale_x_continuous(limits = c(0,1.2), breaks = seq(0,1.25,0.2))+
...
ggplot(df_logit, aes(x = eta)) +
  ...
  labs(
    x = expression(eta[i]),
    y = expression(p[i] == frac(e^{eta[i]}, 1 + e^{eta[i]}))
  ) +
  ylim(-0.1, 1.05) +
  theme_clean(24) +
  guides(
